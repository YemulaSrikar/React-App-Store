...
class AppStore extends Component {
  ...
  filteredApps = () => {
    ...
    const filteredAppsItems = appsList.filter(
      eachItem => eachItem.category === tabsId,
    )
    return filteredAppsItems
  }

  toDisplaySearchInput = () => {
    const {searchInput} = this.state
    const filteredAppsItems = this.filteredApps()
    const filterSearch = filteredAppsItems.filter(eachItem =>
  ...
  render() {
    const filterSearch = this.toDisplaySearchInput()
        ...
        <ul className="tabs-u-list">
          {tabsList.map(eachTabItem => (
            <TabItem
              ...
              isActive={tabsId === eachTabItem.tabId}
            />
          ))}
        </ul>

        <ul className="apps-u-list">
          {filterSearch.map(eachAppItem => (
            <AppItem eachAppDetails={eachAppItem} key={eachAppItem.appId} />
          ))}
        </ul>
      </div>
    )
